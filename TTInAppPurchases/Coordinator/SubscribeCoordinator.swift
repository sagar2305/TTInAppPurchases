...
//
    ...
    class SubscribeCoordinator: Coordinator {
        ...
        init(navigationController: UINavigationController, offeringIdentifier: String? = nil, presented: Bool = true, giftOffer: Bool = false, hideCloseButton: Bool = false, showSpecialOffer: Bool = false) {
            ...
            if presented {
                subscriptionVC = AnnualDiscountedNoTrialViewController()
            } else {
        ...
        private func showTermsOfLaw() {
            let callRecordLawsVC = WebViewController()
            callRecordLawsVC.configureUI(title: "Terms of law".localized)
            callRecordLawsVC.webPageLink = Constants.WebURL.termsOfLaw
        ...
        private func showPrivacyPolicy() {
            ...
            callRecordLawsVC.webPageLink = Constants.WebURL.privacyPolicy
            navigationController.pushViewController(callRecordLawsVC, animated: true)
            navigationController.setNavigationBarHidden(false, animated: true)
        }
        ...
        private func _previousEvent() -> Constants.AnalyticsEvent {
            var event: Constants.AnalyticsEvent = .subscribedBeforeFirstCall
            switch currentEvent {
            case .call:
                if FireStoreHelper.shared.freeCallUsageStatus {
                    event = .subscribedAfterFirstCall
                } else {
                    event = .subscribedBeforeFirstCall
                }
            case .onFirstOnBoardingCompletion:
                event = .subscribedBeforeRegistration
            case .playRecording:
                event = .subscribedAfterPlaying15SecondsOfRecording
            case .transcribeRecording:
                event = .subscribedOnTranscriptionAttempt
            case .shareRecording:
                event = .subscribedOnSharingAttempt
            case .giftOffer:
                event = .subscribedFrom24hrGiftOffer
            }
            ...
            SubscriptionHelper.shared.purchasePackage(product) { [weak self] (success, error) in
                ...
                guard error == nil else {
                    ...
                    case .purchasedFailed:
                        AnalyticsHelper.shared.logEvent(.purchaseFailure,
                                                        properties: [
                                                            .productId: product.product.productIdentifier,
                ...
                if success {
                    ...
                    DispatchQueue.global().async {
                        ...
                        AnalyticsHelper.shared.logEvent(.purchaseComplete,
                                                        properties: [.productId: product.product.productIdentifier,
                                                                     .price: product.price,
                                                                     .previousEvent: self?._previousEvent() ?? "N/A"
                                                        ])
    ...
    extension SubscribeCoordinator: SubscriptionViewControllerDelegate {
        ...
        func exit(_ controller: SubscriptionViewControllerProtocol) {
            ...
            func showSpecialOffer() {
                // TODO enable when you start lifetime plan
...
//                }
    ...
    extension SubscribeCoordinator: SpecialOfferViewControllerDelegate {
        ...
        func didTapCancelButton(_ controller: SpecialOfferViewController) {
            DispatchQueue.global().async {
                AnalyticsHelper.shared.logEvent(.specialOfferCancelled)
            }
            _dismiss()
        }
    ...
    extension SubscribeCoordinator: SpecialOfferUIProviderDelegate {
        ...
        func percentDiscount() -> String {
            guard let annualReducedProduct = availableProducts?.last,
